...
| Catalog | YAML file | REST API endpoints |
...
| `--fields` | All defaults | Comma-separated field codes to extract |
| `--expected-type` | Auto-classify | Skip classification, assume this document type |
| `--expected-nature` | Any | Narrow classification to this nature |
| `--splitting-strategy` | Config default | `visual` or `page_based` |
...
# Skip classification when you know the document type
...
| `--fields` | All defaults | Comma-separated field codes |
| `--expected-type` | Auto-classify | Assume this type for all files |
| `--format` | `json` | Output format: `json`, `table`, `csv` |
...
When using the CLI, document types, fields, and validators are defined in a `catalog.yaml`
file instead of the REST API. Place it in the current directory or pass it explicitly.
...
identical in both modes — only the storage adapters differ.
